 ...
 */
class App {
    constructor({ signingSecret = undefined, endpoints = undefined, agent = undefined, clientTls = undefined, receiver = undefined, convoStore = undefined, token = undefined, botId = undefined, botUserId = undefined, authorize = undefined, orgAuthorize = undefined, logger = undefined, logLevel = undefined, ignoreSelf = true, clientOptions = undefined, processBeforeResponse = false, clientId = undefined, clientSecret = undefined, stateSecret = undefined, installationStore = undefined, scopes = undefined, installerOptions = undefined, } = {}) {
        ...
        if (token !== undefined) {
            if (authorize !== undefined || orgAuthorize !== undefined || usingOauth) {
                throw new errors_1.AppInitializationError(`token as well as authorize, orgAuthorize, or oauth installer options were provided. ${tokenUsage}`);
            }
            ...
            throw new errors_1.AppInitializationError(`No token, no authorize, no orgAuthorize, and no oauth installer options provided. ${tokenUsage}`);
        }
        else if ((authorize !== undefined || orgAuthorize !== undefined) && usingOauth) {
            ...
            this.orgAuthorize = this.receiver.installer.authorize;
        }
        else if (authorize === undefined && orgAuthorize !== undefined && !usingOauth) {
    ...
    async processEvent(event) {
        ...
        if ((type === helpers_1.IncomingEventType.Event &&
            bodyArg.authorizations !== undefined &&
            bodyArg.authorizations[0] !== undefined &&
            bodyArg.authorizations[0].is_enterprise_install) ||
 ...
 */
function buildSource(type, channelId, body, orgInstall) {
    ...
    if (orgInstall) {
        source = {
            teamId: type === helpers_1.IncomingEventType.Event &&
                body.authorizations !== undefined &&
                body.authorizations.length > 0 &&
                body.authorizations[0].team_id !== null
                ? body.authorizations[0].team_id
                : type === helpers_1.IncomingEventType.Event || type === helpers_1.IncomingEventType.Command
                    ? body.team_id
                    : (type === helpers_1.IncomingEventType.Action ||
            ...
            enterpriseId: type === helpers_1.IncomingEventType.Event &&
                body.authorizations !== undefined &&
                body.authorizations.length > 0 &&
                body.authorizations[0].enterprise_id !== null
                ? body.authorizations[0].enterprise_id
                : type === helpers_1.IncomingEventType.Event || type === helpers_1.IncomingEventType.Command
                    ? body.enterprise_id
                    : type === helpers_1.IncomingEventType.Action ||
                        type === helpers_1.IncomingEventType.Options ||
